...
- **`examples/inputs.yaml`** – Example YAML file specifying the path to `anndata` and configurations for `scVI` and `scib-metrics`.  
...
- **`--input_file`** – Path to a YAML file specifying:  
  - `preprocess` section (e.g. `anndata_input`, optional `layer`, `anndata_mask`).  
  - `scvi` section (e.g. `model_input`, `setup_anndata`, `train`).  
  - `scib` section (e.g. `metrics` keys like `batch_key`, `label_key`). 
